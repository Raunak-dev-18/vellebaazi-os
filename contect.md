...
- 2026-03-12: Polished unified chat UX in `Bakaiti` for instant Instagram-like responsiveness: added optimistic message bubbles with background send jobs (non-blocking multi-send), preserved composer focus after send, removed send-lock input disabling, added in-composer sending/done/error status strip, improved message row transitions, and added local pending message state (`sending`/`done`/`failed`) with server-ack dedupe via `clientNonce`. Also added a lightweight `StoryViewer` route fallback page to restore production build stability for the existing `/story/:user_id/:story_id` route import. Files touched: `src/pages/Bakaiti.tsx`, `src/pages/StoryViewer.tsx`, `contect.md`.
- 2026-03-12: Improved full-device responsiveness and adjustable typography: added global fluid root text sizing with a persistent `--app-font-scale` variable (`compact/default/large/xlarge`), applied startup text-scale hydration from local storage, added an Appearance -> Text Size control in Settings (live apply + persisted save), tightened Settings mobile/tablet spacing and wrapping, and added overflow guards in the app shell and Bakaiti viewport sizing for small screens. Also fixed broken JSX fragments in Stories rail rendering that were causing build/lint parse failures. Files touched: `src/index.css`, `src/main.tsx`, `src/pages/Settings.tsx`, `src/App.tsx`, `src/pages/Bakaiti.tsx`, `src/components/Stories.tsx`, `contect.md`.
- 2026-03-12: Implemented dedicated Instagram-style story viewing at `/story/:user_id/:story_id` by activating `StoryViewer` to render the full story viewer, adding route-driven viewer mode in `Stories` (deep-link initialization, URL sync while navigating, blurred background in viewer mode, explicit next/previous story buttons, and close behavior back to home), and fixing viewer JSX/render regressions in the story rail/viewer sections so the new flow builds cleanly. Files touched: `src/pages/StoryViewer.tsx`, `src/components/Stories.tsx`, `contect.md`.
- 2026-03-12: Updated Bakaiti GIF/sticker compose behavior to Instagram-style direct send: tapping a GIF/sticker in the picker now sends immediately (no temporary selected preview row in composer), keeps draft text untouched while media sends in background, and removes GIF-dependent send-button gating so text/file send flow stays clean. Files touched: `src/pages/Bakaiti.tsx`, `contect.md`.
- 2026-03-12: Fixed story deep-link viewer layout regression on `/story/:user_id/:story_id` by hardening viewer-mode dialog positioning (`fixed` full-viewport with transform reset), hiding the extra Radix close button in full-screen mode, and scaling/centering the story card for desktop/mobile so it no longer renders pushed toward the bottom with excess empty space. Files touched: `src/components/Stories.tsx`, `contect.md`.

- 2026-03-12: Refined Bakaiti per-message action controls to match Instagram-style UI: message bubbles now show quick 
eact, 
eply, and 3-dot actions, with Forward, Copy, and Report consolidated inside the overflow menu; updated action icon styling for cleaner visibility and direct-access interaction flow. Files touched: src/pages/Bakaiti.tsx, contect.md.
- 2026-03-12: Refined Bakaiti per-message action controls to match Instagram-like UI references: each message now shows quick React, Reply, and 3-dot actions, while overflow menu actions include Forward, Copy, and Report; also switched the overflow icon to vertical dots and tightened action-button visibility styling for clearer mobile/desktop interaction. Files touched: src/pages/Bakaiti.tsx, contect.md.
- 2026-03-12: Set feed post videos to start unmuted by default by changing PostCard video mute state initialization (isMuted=false), so post audio is on unless user manually mutes. Files touched: src/components/PostCard.tsx, contect.md.
- 2026-03-12: Fixed persistent story deep-link misalignment by adding a dedicated viewer-mode DialogContent override (`data-story-viewer="true"`) that force-resets Radix modal positioning/transform/animation and hides the built-in close button for full-screen story pages; also switched viewer card sizing to width-based (`w-[92vw] max-w-[420px] aspect-[9/16]`) for stable center alignment on narrow/mobile screens. Files touched: `src/components/Stories.tsx`, `src/index.css`, `contect.md`.
- 2026-03-12: Upgraded story editor UX: fixed @mention suggestion layering by making the text tool panel non-clipping with elevated input z-order, added GIF/sticker insertion inside the Story Stickers tab via Giphy picker, and added Instagram-style sticker selection with corner drag-resize handles for both emoji and media stickers (including proper media-sticker rendering during story export). Files touched: src/components/StoryEditor.tsx, contect.md.
- 2026-03-12: Refined Bakaiti message-action visibility and composer status UX: removed the always-visible 'Sending in background' composer strip (composer now shows only done/error feedback), and made per-message React/Reply/More controls appear only when hovering/focusing that specific message bubble. Files touched: src/pages/Bakaiti.tsx, contect.md.
- 2026-03-12: Applied mobile-first Bakaiti/chat shell refinements for smoother Android UX: hid global mobile bottom-nav on /bakaiti to remove chat-screen obstruction, made route content bottom padding adaptive when nav is hidden, optimized chat autoscroll via requestAnimationFrame with mobile auto-scroll behavior, reduced message transition cost, added overscroll/touch containment to chat and conversation panes, added safe-area-aware composer padding/backdrop treatment, and enabled lazy/async decoding for inline chat images/shared-post previews. Files touched: src/App.tsx, src/pages/Bakaiti.tsx, contect.md.
- 2026-03-12: Final pre-production safety pass for Bakaiti mobile actions: kept message action controls hover-reveal on desktop while restoring touch usability on mobile by scoping hide/reveal behavior to md: breakpoints (prevents action controls becoming unreachable on touch devices). Re-validated with lint/build/type-check. Files touched: src/pages/Bakaiti.tsx, contect.md.
- 2026-03-12: Implemented notification deep-link routing so tapping a notification opens the exact target context: story notifications open `/story/:owner/:storyId`, group mentions open the specific group conversation in `Bakaiti`, DM-linked notifications open the target DM thread, and post/comment mentions open Home with post/comment query targets. Added Home feed deep-link handling to auto-scroll to target post, auto-open comments for comment targets, and highlight the relevant comment/reply anchor; also extended Bakaiti route-state handling to accept `openConversation` (`dm`/`group`) for direct navigation from notifications. Files touched: `src/pages/Notifications.tsx`, `src/pages/Home.tsx`, `src/pages/Bakaiti.tsx`, `contect.md`.
- 2026-03-12: Fixed Bakaiti mobile chat clipping/cutoff by hardening the chat layout for small screens: added `w-full` + `min-h-0/min-w-0` + overflow guards on root/list/chat panes, made list/chat scroll areas `min-h-0` for proper flex shrinking, improved mobile header overflow handling (truncated title, non-shrinking badges/actions), and made composer row/input explicitly `w-full` + `min-w-0 flex-1` so input/actions no longer get cut on narrow devices. Files touched: `src/pages/Bakaiti.tsx`, `contect.md`.
- 2026-03-12: Added Instagram-style URL link previews in Bakaiti messages: when a message contains an HTTP(S) link, the app now fetches metadata through a new same-origin `/api/link-preview` proxy endpoint and renders a preview card with favicon, site name, title, description, and OpenGraph image inside the chat bubble (with loading state + URL safety checks for private/local hosts). Files touched: `vite.config.ts`, `src/pages/Bakaiti.tsx`, `contect.md`.
- 2026-03-13: Performed production security hardening pass: strengthened `/api/link-preview` against abuse with IP rate limiting, timeout-controlled fetches, safe manual redirect handling, cache TTL, oversized-response guard, and strict URL validation (protocol/credentials/private-host checks); and hardened Bakaiti message rendering by sanitizing all dynamic media/link URLs to allow only `http/https` before rendering attachments, shared-post media, and preview links/icons/images. Files touched: `vite.config.ts`, `src/pages/Bakaiti.tsx`, `contect.md`.
- 2026-03-17: Redesigned the desktop sidebar to an Instagram-style icon rail with compact left-rail spacing, icon-first nav buttons, unread badge dots for Bakaiti/Notifications, bottom utility actions, and profile avatar action while preserving existing route mapping. Files touched: `src/components/AppSidebar.tsx`, `contect.md`.
- 2026-03-17: Fixed emoji rendering corruption in Bakaiti reaction picker by replacing legacy mojibake quick-reaction literals with explicit Unicode codepoint escapes, ensuring consistent emoji display across devices/locales/build environments. Files touched: `src/pages/Bakaiti.tsx`, `contect.md`.
- 2026-03-17: Applied an Instagram-like visual polish pass across feed/chat surfaces: refined Home feed shell and right-rail spacing/link styling, upgraded PostCard hierarchy (header/actions/caption/time/composer rhythm), tightened Stories rail proportions/rings for mobile-first consistency, tuned desktop icon-rail sidebar spacing/shape, and hardened Bakaiti reaction emoji rendering with explicit cross-platform emoji font fallback. Files touched: `src/pages/Home.tsx`, `src/components/PostCard.tsx`, `src/components/Stories.tsx`, `src/components/AppSidebar.tsx`, `src/pages/Bakaiti.tsx`, `src/index.css`, `contect.md`.
- 2026-03-17: Fixed desktop sidebar scroll behavior by pinning sidebar wrapper to the viewport (`fixed inset-y-0 left-0`) and offsetting app content (`md:ml-[74px]`) so the rail stays stationary while feed content scrolls. Files touched: `src/App.tsx`, `contect.md`.
- 2026-03-22: Updated Bakaiti message action UX to side-of-bubble controls (Instagram-like): moved React/Reply/More controls from below bubbles to a side rail beside each message (hover-reveal on desktop, directly available on touch/mobile), and added a sender-only Delete action in the overflow menu with realtime removal support for DM/group messages. Files touched: `src/pages/Bakaiti.tsx`, `contect.md`.
