...
import { Grid3x3, Bookmark, UserSquare2, Settings, ArrowLeft, Video } from "lucide-react";
...
export default function UserProfile() {
  ...
  const [userData, setUserData] = useState({
    ...
    fullName: username?.replace(/[._]/g, " ").replace(/\d+/g, "").trim() || "User",
    ...
    const fetchUserData = async () => {
      ...
      try {
        ...
        if (snapshot.exists()) {
          ...
          if (userEntry) {
            const [uid, data]: [string, any] = userEntry;
            ...
            const followingCount = followingCountSnapshot.exists() ? Object.keys(followingCountSnapshot.val()).length : 0;
            ...
            if (canView) {
              ...
              if (postsSnapshot.exists()) {
                ...
                postsData = Object.entries(allPosts)
                  .filter(([_, post]: [string, any]) => post.userId === uid)
            ...
            setUserData({
              ...
              fullName: data.username?.replace(/[._]/g, " ").replace(/\d+/g, "").trim() || "User",
              avatar: data.photoURL || `https://api.dicebear.com/7.x/avataaars/svg?seed=${username}`,
              bio: data.bio || "",
  ...
  const handleFollowToggle = async () => {
    ...
    try {
      ...
      if (isFollowing) {
        ...
        setUserData(prev => ({ ...prev, followers: Math.max(0, prev.followers - 1) }));
        ...
        if (isPrivateAccount) {
          ...
          const followRequestRef = push(ref(db, `followRequests/${userData.uid}`));
          ...
          // Create notification
          const notificationRef = push(ref(db, `notifications/${userData.uid}`));
          ...
          if (isPrivateAccount) {
            ...
            if (postsSnapshot.exists()) {
              const allPosts = postsSnapshot.val();
              postsData = Object.entries(allPosts)
                .filter(([_, post]: [string, any]) => post.userId === userData.uid)
                .map(([id, post]: [string, any]) => ({
                  id,
                  ...post
                }))
        ...
        <div className="flex items-start gap-8 mb-6">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-4 mb-6">
              ...
              <Button 
                className={isFollowing ? "bg-secondary hover:bg-secondary/80 text-foreground px-8" : "bg-blue-500 hover:bg-blue-600 text-white px-8"}
        ...
        <TabsContent value="posts" className="mt-0">
          {!canViewPosts ? (
            <div className="flex flex-col items-center justify-center py-20">
              ...
              <h3 className="text-2xl font-light mb-2">This Account is Private</h3>
            ...
            <div className="flex flex-col items-center justify-center py-20">
              ...
              <p className="text-muted-foreground text-sm">
                When {userData.username} posts, you'll see their photos and videos here.
              </p>
            </div>
          ) : (
            <>
