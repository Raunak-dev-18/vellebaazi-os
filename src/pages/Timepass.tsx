import { Heart, MessageCircle, Send, Bookmark, Volume2, VolumeX, Music, MoreHorizontal, X } from "lucide-react";
...
export default function Timepass() {
    ...
    try {
      ...
      if (snapshot.exists()) {
        ...
        const postsArray = Object.entries(allPosts)
          ...
          .filter((post: Post) => post.mediaType === 'video' || post.postType === 'reel' || post.mediaType === 'image')
      ...
      if (snapshot.exists()) {
        ...
        const commentsArray = Object.entries(commentsData)
          .map(([id, comment]: [string, any]) => ({
            ...
            repliesCount: comment.replies ? Object.keys(comment.replies).length : 0,
            repliesList: comment.replies ? Object.entries(comment.replies)
              .map(([replyId, reply]: [string, any]) => ({
                id: replyId,
                ...reply,
                likesCount: reply.likes ? Object.keys(reply.likes).length : 0
              }))
  ...
  const handleLikeComment = async (commentId: string, isReply = false, parentCommentId?: string) => {
    ...
    try {
      const db = getDatabase();
      const likePath = isReply && parentCommentId
  ...
  const handleAddComment = async () => {
    ...
    try {
      ...
      if (replyingTo) {
        // This is a reply
        const repliesRef = ref(db, `comments/${selectedPostId}/${replyingTo.commentId}/replies`);
      ...
      <div className="h-screen overflow-y-scroll snap-y snap-mandatory bg-background scrollbar-hide">
        ...
        <div key={post.id} className="h-screen snap-start flex items-center justify-center">
          <div className="w-full h-full flex items-center justify-center gap-6 px-4 relative">
            {/* Media Card */}
            <div className={`${post.mediaType === 'video' ? 'w-[400px] h-[calc(100vh-8rem)]' : 'w-[400px] h-[400px]'} bg-card rounded-2xl overflow-hidden shadow-xl border border-border relative flex-shrink-0`}>
              {post.mediaType === 'video' ? (
                ...
                >
                  ...
                  >
                    {mutedVideos.has(post.id) || !videoRefs.current[post.id]?.muted === false ? (
              ...
              <div className="absolute bottom-4 left-4 right-4 text-white space-y-2">
                ...
                >
                  ...
                  <span className="font-semibold text-sm drop-shadow-lg">{post.username}</span>
                  {post.userId !== user?.uid && (
                    <>
                      <span className="text-sm drop-shadow-lg">•</span>
                      <Button variant="ghost" size="sm" className="h-auto p-0 text-white font-semibold hover:bg-transparent hover:text-white text-sm">
                        Follow
                      </Button>
                    </>
                  )}
                ...
                {post.taggedUsers && post.taggedUsers.length > 0 && (
                  <div className="flex items-center gap-1 text-xs mt-1">
                    ...
                    {post.taggedUsers.slice(0, 2).map((tag, i) => (
                      <span key={i} className="drop-shadow-lg">@{tag}</span>
                    ))}
                    {post.taggedUsers.length > 2 && (
                      <span className="drop-shadow-lg">+{post.taggedUsers.length - 2} more</span>
                    )}
                  </div>
                )}
              </div>
            ...
            <div className="flex flex-col gap-8 items-center justify-end h-[calc(100vh-8rem)] flex-shrink-0 pb-4">
              ...
              {post.commentsEnabled && (
                <div className="flex flex-col items-center gap-1">
                  ...
                  >
                    <MessageCircle className="h-7 w-7 stroke-[2.5]" strokeWidth={2.5} />
                  </button>
                  <span className="text-xs font-semibold">{formatCount(post.comments)}</span>
                </div>
              )}
              ...
              <button className="p-0 hover:scale-110 transition-transform">
                <Bookmark className="h-7 w-7 stroke-[2.5]" strokeWidth={2.5} />
              </button>

              <button className="p-0 hover:scale-110 transition-transform">
                <MoreHorizontal className="h-7 w-7 stroke-[2.5]" strokeWidth={2.5} />
              </button>
            </div>
                        ...
                        <div key={comment.id} className="space-y-2">
                          ...
                          <div className="flex gap-3">
                            <Avatar 
                              ...
                              onClick={() => {
                                closeCommentModal();
                                navigate(`/users/profile/${comment.username}`);
                              }}
                            >
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-sm mt-1"><MentionText text={comment.text} /></p>
                                  ...
                                  {comment.repliesList.map((reply: any) => (
                                    <div key={reply.id} className="flex gap-2">
                                      <Avatar 
                                        ...
                                        onClick={() => {
                                          closeCommentModal();
                                          navigate(`/users/profile/${reply.username}`);
                                        }}
                                      >
                                      ...
                                      <div className="flex-1 min-w-0">
                                        ...
                                        <div className="flex items-center gap-3 mt-1">
                                          ...
                                          <button
                                            onClick={() => handleReplyToComment(comment.id, reply.username)}
                                            className="text-xs text-muted-foreground font-medium hover:text-foreground"
                                          >
                                            Reply
                                          </button>
                ...
                <div className="border-t border-border p-4">
                  ...
                  <div className="flex gap-2">
                    <Avatar className="h-8 w-8">
                      <AvatarImage src={user?.photoURL || `https://api.dicebear.com/7.x/avataaars/svg?seed=${user?.uid}`} alt="You" />
